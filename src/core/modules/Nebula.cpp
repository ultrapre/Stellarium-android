...
QString Nebula::getInfoString(const StelCore *core, const InfoStringGroup& flags) const
{
    ...
    if (posAngle >= 0 && posAngle <= 180 && flags&posAngle1)
        oss << QString("Position angle: %1 °<br>").arg(posAngle);

    if (typeClass != "" && flags&typeClass1)
        oss << QString("detailed type: %1<br>").arg(typeClass);
//    if (dsp != "" && flags&dsp1)
...
void Nebula::drawLabel(StelPainter& sPainter, float maxMagLabel)
{
    ...
    else
    {
        if (M_nb > 0)
            ...
            str = QString("N%1").arg(NGC_nb);
        else if (IC_nb > 0)
            str = QString("I%1").arg(IC_nb);
    }
...
void Nebula::readNGC(QDataStream& in)
{
    ...
    in >> isIc >> nb >> ra >> dec >> mag >> angularSize >> type >> bmag >> bV >> surfBright >> posAngle >> typeClass >> dsp;
    if (isIc)
    {
        IC_nb = nb;
    }
